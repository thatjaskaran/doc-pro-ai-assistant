...
export function IntakeForm() {
  ...
  const [stage, setStage] = useState<'screening' | 'chat' | 'redflag' | 'results'>('screening');
  ...
  // STAGE: RED FLAG WARNING
  if (stage === 'redflag') {
    return (
      <div
        role="alert"
        className="mx-auto max-w-2xl rounded-3xl border border-red-200 bg-red-50/60 p-8 text-center shadow-sm backdrop-blur-sm animate-in fade-in duration-300"
      >
        ...
        <h2 className="text-xl font-serif font-bold text-red-950">Please Seek Care Immediately</h2>
        <p className="mt-3 text-sm leading-relaxed text-red-800">{redFlagGuidance}</p>
      </div>
    );
  }

  // STAGE: INITIAL SCREENING CHECKBOXES
  if (stage === 'screening') {
    return (
      <form
        onSubmit={handleScreeningSubmit}
        className="mx-auto max-w-2xl space-y-6 rounded-3xl border border-slate-200/80 bg-white p-6 sm:p-8 shadow-sm transition animate-in fade-in duration-300"
      >
        <fieldset className="space-y-4">
          <legend className="text-base font-serif font-bold text-slate-900 leading-snug">
            ...
            {RED_FLAG_RULES.map((rule) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="text-xs sm:text-sm font-medium leading-relaxed">{rule.label}</span>
                </label>
              );
            })}
          </div>
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-4 animate-in fade-in duration-300">
      {/* Messages Window */}
      <div className="h-[420px] overflow-y-auto rounded-3xl border border-slate-200/80 bg-white p-4 sm:p-6 shadow-sm space-y-4">
        {messages.map((m, i) => {
