...
interface ApiAssistResponse {
    redFlag?: boolean;
    guidance?: string;
    error?: string;
    reply?: string;
    result?: AiGuidanceResult;
...
export function IntakeForm() {
    ...
    const [redFlagGuidance, setRedFlagGuidance] = useState<string>('');
    ...
    async function handleScreeningSubmit(e: React.FormEvent<HTMLFormElement>): Promise<void> {
        e.preventDefault();
        ...
        try {
            ...
            const data = (await res.json()) as ApiAssistResponse;

            setShowScreeningModal(false);

            if (data.redFlag) {
                setRedFlagGuidance(data.guidance ?? 'Please seek immediate medical care.');
    ...
    async function handleGetSummary(): Promise<void> {
        ...
        try {
            ...
            if (data.result && data.suggestedDoctors) {
    ...
    function handleBookWithDoctor(doctorId: string): void {
                        ...
                        <form onSubmit={handleScreeningSubmit} className="space-y-6">
                            <fieldset className="space-y-4">
                                ...
                                <div className="space-y-2.5 pt-2">
                                    {RED_FLAG_RULES.map((rule: { id: string; label: string }) => {
                        ...
                        {suggestedDoctors.length === 0 ? (
                            ...
                            <div className="grid gap-4 sm:grid-cols-2">
                                {suggestedDoctors.map((doctor) => {
                                    const rating = doctor.ratingAverage !== null && doctor.ratingAverage !== undefined
                                        ...
                                        >
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-xs text-amber-600 font-medium">
                                                    {doctor.ratingCount > 0 && rating
                                                        ? `★ ${rating} (${doctor.ratingCount} reviews)`
                                                        : 'No reviews yet'}
                                                </p>
                                            </div>
                                            <button
                ...
                <div className="mx-auto max-w-2xl space-y-4 animate-in fade-in duration-300">
                    ...
                    >
                        {messages.map((m: ChatMessage, i: number) => {
                            ...
                            return (
                                <div key={`chat-msg-${i}`} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
